...
int main(int argc, char * const argv[]) {
	...
	bool winnow_first_round_true = false;
	bool alternate = false;
	float winnow_init_wgts = 0.0;
	algorithm alg = algorithm::SORCAR;
	
	// Parse
	int opt;
	while ((opt = getopt(argc, argv, "a:frtw:")) != -1)
	{
		...
		switch(opt)
		{
			...
			case 'f':
			{
				...
				// Make winnow propose true in first round
				winnow_first_round_true = true;
				break;
			}
			...
			// Get initial weights of winnow
			case 'w':
			{
				std::string arg(optarg);
				winnow_init_wgts = std::stof(arg);
				std::cout << "Initial winnow wgts: " << winnow_init_wgts << std::endl;
				break;
			}
	...
	else if (alg == algorithm::WINNOW)
	{
		...
		{
			winnow_objs.push_back(winnow(intervals[i].second - intervals[i].first + 1, winnow_init_wgts));
		}
		...
		if (round != 1)
		{
			...
			winnow::execute_algorithm(winnow_objs, datapoints, X, R);	
			//winnow::execute_algorithm(winnow_objs, datapoints, R);	
		}

		if (!winnow_first_round_true && round == 1)
		{
			winnow::execute_algorithm(winnow_objs, datapoints, X, R);
			//winnow::execute_algorithm(winnow_objs, datapoints, X);	
		}


		//winnow::write_ltf2bool_json(winnow_objs, metadata, file_stem + ".json", (winnow_first_round_true && round == 1));
