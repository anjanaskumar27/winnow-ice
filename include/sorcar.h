...
namespace horn_verification
{
	...
	class winnow
	{
		...
		template <class T>
		static void execute_algorithm(std::vector<winnow> &w_objs, const std::vector<datapoint<T>> &dps, std::vector<std::set<unsigned>> &X, const std::vector<horn_constraint<bool>> & horn_constraints)
		{
			...
			// Populate P and N samples in winnow objs
 			for (auto dp : dps)
			{	
				//std::cout << "\nDatapoints, label: " << dp._classification << " is classified " << dp._is_classified << std::endl;			
				if ( !dp._is_classified )
				{
					continue;
				}
			...
			// Train winnow on P and N and find ltf
			for (unsigned i = 0; i < w_objs.size(); ++i)
			{
				w_objs[i].train(winnow_dps[i]);
			}
			//std::cout << "\nMilestone 0:Trained winnow on P and N: Winnow dp size "<< winnow_dps[0].size() << " Data size " << dps.size() << std::endl;
			int flag_impl_sat = 1;
			// Repeat until all ICE samples are satisfied
			do
			{
				// Reset w_objs dps
				for (unsigned i = 0; i < winnow_dps.size(); ++i)
				{
					winnow_dps[i].clear();
				}
				//counter = dps.size();
				//std::cout << "\nMilestone 1 "<< std::endl;
				// Populate P and N samples in winnow objs
				for (auto dp : dps)
				{				
					if (!dp._is_classified)
				...
				//std::cout << "\nMilestone 2 "<< std::endl;
				flag_impl_sat = 1;
				// Iterate through the implication samples.
				for (const auto & hc : horn_constraints)
				{
					datapoint<T> dp_s = *hc._premises[0];
					datapoint<T> dp_sprime = *hc._conclusion;
					//std::cout << "\n HC dp "<< *hc._conclusion << " " << dp_sprime <<std::endl;
					//std::cout << "\nHC: Sprime isclassified " <<dp_sprime._is_classified << " S isclassified " << dp_s._is_classified <<std::endl;
					//For every (s,s’) where s is classified true and s’ is classified false by current LTF
					if (w_objs[0].predict(dp_s) && !w_objs[0].predict(dp_sprime))
					{
						flag_impl_sat = 0;
						// If Houdini (X) labels s as positive (and hence s’ as positive), add s’ to P
						if(sorcar::satisfies(dp_s, X[dp_s._categorical_data[0]]))
						{
							//std::cout << "\nB0 : counter" << counter<< std::endl;
							dp_sprime._classification = true;
							if (!dp_sprime._is_classified)
							{
								dp_sprime._is_classified = true;
								winnow_dps[dp_sprime._categorical_data[0]].push_back(dp_sprime);
								//counter--;
								//std::cout << "\nB1 : counter" << counter<< std::endl;
							}

							if (!dp_s._is_classified)
							{
								//counter--;
								std::cout << "\n B1 SHOULD NOT OCCUR ERR" << std::endl;
							}
						}
						else
						{
							//std::cout << "\nB3 : counter" << counter<< std::endl;
							dp_s._classification = false;
							if (!dp_s._is_classified)
							{
								dp_s._is_classified = true;
								winnow_dps[dp_s._categorical_data[0]].push_back(dp_s);
								//counter--;
								//std::cout << "\nB4 : counter" << counter<< std::endl;
							}
							if (!dp_sprime._is_classified)
							{
								//counter--;
								std::cout << "\nB2 SHOULD NOT OCCUR ERR" << std::endl;
								//std::cout << "\nB5 : counter" << counter<< std::endl;
							}
						}
						break;
					}
				}
				//std::cout << "\nMilestone 3 "<< std::endl;
				for(unsigned int i = 0; i < w_objs.size(); ++i)
				{
					for(unsigned int j = 0; j < winnow_dps[i].size(); ++j)
					{
						// Invert data and label
						for (unsigned int k = 0; k < winnow_dps[i][j]._int_data.size(); ++k)
						{
							winnow_dps[i][j]._int_data[k] = 1 - winnow_dps[i][j]._int_data[k];
						}
						winnow_dps[i][j]._classification = !winnow_dps[i][j]._classification;
					}
				}
					
				//std::cout << "\nMilestone 4 : counter :" << counter << std::endl;
				for (unsigned i = 0; i < w_objs.size(); ++i)
				{
					w_objs[i].train(winnow_dps[i]);
				}
			} while(flag_impl_sat == 0);
	
			//std::cout << "\nNumber of data points second LTF "<< winnow_dps[0].size() << std::endl;
